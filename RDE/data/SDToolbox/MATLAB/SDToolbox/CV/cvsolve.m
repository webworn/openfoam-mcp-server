...
% cvsolve:  computes solutions for constant-volume, adiabatic explosion model equations.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% create solution vector for ODE solver
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
tel = [0 t_end];
options = odeset('RelTol',rel_tol,'AbsTol',abs_tol,'max_step',max_step,'Stats','off');
% t0 = cputime;

% out.x = time, out.y = temperature and species profiles
out = ode15s(@cvsys,tel,y0,options,gas,mw,R0);
...
for i = 1:b
    T = out.y(1,i);
    Y = out.y(2:end,i);
    set(gas, 'T', T, 'Rho', R0, 'Y', Y');
    P = pressure(gas);
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    % COMPUTE TEMPERATURE time derivative using energy equation form from cvsys.m                              % 
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

    wdot = netProdRates(gas);
    temp_grad(i) = - temperature(gas) * gasconstant * (enthalpies_RT(gas) - ones(nsp,1))' ...
       * wdot / (R0*cv_mass(gas));
...
elseif (n == 1)
    ...
    max_temp_grad = max(temp_grad ); % max temperature gradient
    half_T_flag1 = 0;
    half_T_flag2 = 0;

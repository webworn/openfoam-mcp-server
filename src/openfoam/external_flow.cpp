...
void ExternalFlowAnalyzer::validateInput(const ExternalFlowInput& input)
{
    ...
    {
        throw std::invalid_argument("Velocity too high (>340 m/s), supersonic flow not yet supported");
    }
    
    if (input.characteristicLength > 100)
    {
        throw std::invalid_argument("Characteristic length too large (>100 m), please verify input");
    }
    
    if (input.vehicleType != "car" && input.vehicleType != "aircraft" && 
        input.vehicleType != "building" && input.vehicleType != "cylinder")
    {
        throw std::invalid_argument("Vehicle type must be 'car', 'aircraft', 'building', or 'cylinder'");
    }
}
...
ExternalFlowResults ExternalFlowAnalyzer::processResults(const std::string& caseId, const ExternalFlowInput& input)
{
    ExternalFlowResults results;
...
double ExternalFlowAnalyzer::calculateTheoreticalDragCoefficient(const ExternalFlowInput& input) const
{
    ...
    {
        return getBuildingDragCoefficient(Re, input.characteristicLength / std::sqrt(input.frontalArea));
    }
...
double ExternalFlowAnalyzer::getAircraftDragCoefficient(double Re, double Ma, const std::string& airfoil_type) const
{
...
std::string ExternalFlowAnalyzer::determineCompressibilityRegime(const ExternalFlowInput& input) const
{

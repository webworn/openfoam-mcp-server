...
PipeFlowAnalyzer::PipeFlowAnalyzer()
...
PipeFlowResults PipeFlowAnalyzer::processResults(const std::string& caseId, const PipeFlowInput& input)
{
    PipeFlowResults results;
    ...
    results.pressureDrop = calculateTheoreticalPressureDrop(input);
    results.wallShearStress = results.frictionFactor * 0.5 * input.density * input.velocity * input.velocity;
...
double PipeFlowAnalyzer::calculateTheoreticalPressureDrop(const PipeFlowInput& input) const
{
    ...
    return f * (input.length / input.diameter) * 0.5 * input.density * input.velocity * input.velocity;
}
...
std::string PipeFlowAnalyzer::generateAnalysisReport(const PipeFlowInput& input, const PipeFlowResults& results) const
{
    ...
    report << "  Reynolds Number: " << std::fixed << std::setprecision(0) << results.reynoldsNumber << std::endl;
    report << "  Flow Regime: " << results.flowRegime << std::endl;
    report << "  Friction Factor: " << std::fixed << std::setprecision(4) << results.frictionFactor << std::endl;
    report << "  Pressure Drop: " << std::fixed << std::setprecision(2) << results.pressureDrop << " Pa" << std::endl;
    report << "  Wall Shear Stress: " << std::fixed << std::setprecision(3) << results.wallShearStress << " Pa" << std::endl;
    report << std::endl;

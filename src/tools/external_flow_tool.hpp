...
private:
    ...
    std::string generatePhysicsExplanation(const ExternalFlowInput& input, const ExternalFlowResults& results) const;
    std::string generateRecommendations(const ExternalFlowInput& input, const ExternalFlowResults& results) const;
    std::string generateApplicationGuidance(const ExternalFlowInput& input, const ExternalFlowResults& results) const;
...
public:
    ...
    {
        return "Analyze external flow around vehicles, aircraft, and buildings using OpenFOAM CFD simulation. "
               "Calculates drag coefficient, lift coefficient, Reynolds number, and aerodynamic forces for "
               "automotive, aerospace, and civil engineering applications. Supports cars, aircraft, buildings, "
               "and cylinders with automatic flow regime detection and expert aerodynamics guidance.";
    }

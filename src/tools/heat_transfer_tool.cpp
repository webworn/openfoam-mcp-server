...
HeatTransferTool::HeatTransferTool()
                                                ...
                                                const HeatTransferResults& results) const
{
    ...
    summary << "• Convective: " << std::fixed << std::setprecision(1) 
            << results.convectiveHeatTransfer << " W (" 
            << std::setprecision(0) << (results.convectiveHeatTransfer/results.totalHeatTransferRate)*100 << "%)\n";
    summary << "• Conductive: " << std::fixed << std::setprecision(1) 
            << results.conductiveHeatTransfer << " W (" 
            << std::setprecision(0) << (results.conductiveHeatTransfer/results.totalHeatTransferRate)*100 << "%)\n";
    summary << "• Radiative: " << std::fixed << std::setprecision(1) 
            << results.radiativeHeatTransfer << " W (" 
            << std::setprecision(0) << (results.radiativeHeatTransfer/results.totalHeatTransferRate)*100 << "%)\n\n";
    
    summary << (results.success ? "✅" : "❌") << " **OpenFOAM Simulation Status:** ";
    summary << (results.success ? "Completed successfully" : "Failed - using theoretical calculations") << "\n";
    ...
    content.push_back(json{{"type", "text"}, {"text", generateDesignRecommendations(input, results)}});
    
    // Application-specific guidance
    content.push_back(json{{"type", "text"}, {"text", generateApplicationGuidance(input, results)}});
    
    // Add warning if OpenFOAM failed
    if (!results.success && !results.errorMessage.empty())
    {
        content.push_back(json{{"type", "text"}, {"text", "⚠️  OpenFOAM simulation failed, but theoretical calculations were provided."}});
        content.push_back(json{{"type", "text"}, {"text", "Error details: " + results.errorMessage}});
    }
    
    // JSON resource with detailed results
    std::string resourceUri = results.success ? 
        "openfoam://heat_transfer/" + results.caseId : 
...
std::string HeatTransferTool::generateDesignRecommendations(const HeatTransferInput& input, 
                                                          const HeatTransferResults& results) const
{
...
json HeatTransferTool::getInputSchema()
{
    return json{
            ...
            {"properties", json{
                {"analysisType", json{
                    {"type", "string"},
                    {"enum", json::array({"electronics_cooling", "heat_exchanger", "building", "engine"})},
                    {"default", "electronics_cooling"},
                    {"description", "Type of heat transfer analysis"}
                }},
                {"characteristicLength", json{
                    {"type", "number"},
                    {"minimum", 0.001},
                    {"maximum", 10.0},
                    {"description", "Characteristic length in m (chip size, tube diameter, room dimension)"}
                }},
                {"heatGeneration", json{
                    {"type", "number"},
                    {"minimum", 0.1},

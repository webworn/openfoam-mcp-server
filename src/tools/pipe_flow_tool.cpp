...
std::string PipeFlowTool::formatResultsForUser(const PipeFlowResults& results) const {
    ...
    output << "• Average Velocity: " << std::fixed << std::setprecision(2)
           << results.averageVelocity << " m/s\n";
    output << "• Maximum Velocity: " << std::fixed << std::setprecision(2) << results.maxVelocity
           << " m/s\n\n";

    output << "**Pressure and Friction:**\n";
    output << "• Friction Factor: " << std::fixed << std::setprecision(4) << results.frictionFactor
           << "\n";
    output << "• Pressure Drop: " << std::fixed << std::setprecision(2) << results.pressureDrop
           << " Pa";
    if (results.pressureDrop > 1000) {
        output << " (" << std::fixed << std::setprecision(1) << results.pressureDrop / 1000
               << " kPa)";
    }
    output << "\n";
    output << "• Wall Shear Stress: " << std::fixed << std::setprecision(3)
           << results.wallShearStress << " Pa\n\n";
...
std::string PipeFlowTool::generatePhysicsExplanation(const PipeFlowInput& input,
    ...
    explanation << "• Reynolds Number: Re = ρVD/μ = " << std::fixed << std::setprecision(0)

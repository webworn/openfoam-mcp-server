...
public:
    ...
    // Scheme selection
    std::map<std::string, std::string> selectNumericalSchemes(
        PhysicsCategory physics, 
...
namespace ScenarioExamples
{
    ...
    const std::vector<std::string> PROCESS_SCENARIOS = {
        "Optimize mixing in stirred tank reactor",
    ...
    const std::vector<std::string> MARINE_SCENARIOS = {
        "Analyze ship hull resistance at design speed",
    ...
    const std::vector<std::string> CIVIL_SCENARIOS = {
        "Calculate wind loads on 50-story skyscraper",
    ...
    const std::vector<std::string> BIOMEDICAL_SCENARIOS = {
        "Analyze blood flow through coronary artery",
    ...
    const std::vector<std::string> ENVIRONMENTAL_SCENARIOS = {
        "Study atmospheric pollution dispersion",
